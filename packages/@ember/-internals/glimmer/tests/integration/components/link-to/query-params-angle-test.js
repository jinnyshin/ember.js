...
if (EMBER_GLIMMER_ANGLE_BRACKET_BUILT_INS) {
  moduleFor(
    ...
    class extends ApplicationTestCase {
      ...
      ['@test populates href with partially supplied query param values, but omits if value is default value']() {
        this.addTemplate(
          'index',
          `<LinkTo @route='index' @query={{hash foo='123'}}>Index</LinkTo>`
        );
  ...
  moduleFor(
    ...
    class extends ApplicationTestCase {
      ...
      ['@test The <LinkTo /> component disregards query-params in activeness computation when current-when is specified'](
        assert
      ) {
        let appLink;
